...
Xcode 工程只负责应用外壳、资源和 UI 测试。可复用逻辑放在本地 Swift Package `CodexQuotaKit`，不引入运行时第三方依赖。
...
- `macos/Scripts/build-universal.sh` 在 Xcode 命令行工具可用时生成 Universal `.app`。
...
- Xcode UI 测试验证菜单、设置和权限说明文案。

### 9.3 CI

- GitHub Actions `macos-15` runner 执行 Swift Package 测试和 Xcode 测试。
