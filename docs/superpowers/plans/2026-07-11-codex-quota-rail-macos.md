...
**Architecture:** 领域模型、设置、协议客户端和窗口放置位于本地 Swift Package `CodexQuotaKit`；根 Swift Package 的原生可执行 Target 负责 Accessibility 窗口跟踪、`NSPanel`、`NSStatusItem` 与 `SMAppService`。macOS GitHub Actions 执行测试、Universal 构建、ad-hoc 签名与 ZIP 校验。
...
Run: `swift build --package-path macos --configuration debug`
...
```bash
git add macos/App macos/Resources macos/Package.swift macos/Packages/CodexQuotaKit
...
Expected: `swift test` 与原生 Target 编译通过；`lipo -archs` 同时输出 `arm64 x86_64`；ZIP 可解包且 ad-hoc 签名验证通过。
